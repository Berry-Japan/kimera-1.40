...
protected:
  ...
  void sendXIMSetEventMask(u_int16_t im, u_int16_t ic, ulong fwrd = 0, ulong sync = 0) const;
  void sendXIMRegisterTriggerkeys(u_int16_t im) const;
  void sendXIMSync(u_int16_t im, u_int16_t ic) const;
  void sendXIMError(u_int16_t im, u_int16_t ic, XIMErrorFlag flag, XIMErrorCode error) const;
  void sendXIMError(Window client, XIMErrorCode error) const;

  // Callbacks
  void sendXIMPreeditStart(u_int16_t im, u_int16_t ic) const;
  void recvXIMPreeditStartReply() const;
  void sendXIMPreeditDraw(u_int16_t im, u_int16_t ic, int caret, int index, int length, const QString& str=QString::null, const QByteArray& feedback=QByteArray()) const;
  void sendXIMPreeditDone(u_int16_t im, u_int16_t ic) const;
  ...
  void sendClientMessage(u_int16_t im, const QByteArray&) const;
  void sendPropertywithCM(u_int16_t im, const QByteArray&) const;
 
  QPoint     calcPreeditPoint(u_int16_t im, u_int16_t ic, bool& ok) const;
  bool       checkCommWindow(u_int16_t im) const;
  int        pad(int n) const { return (4 - (n % 4)) % 4; }
  u_int16_t  numElements(int n) const { return (n + pad(n)) / 4; }
...
private:
  ...
  QValueStack<Window> _cltwinstack; // current client communication window ID
                                    //   for XTransportConnection, XIMOpen, XIMClose,
                                    //   XIMConnect and XIMDisconnect messages.
  u_int16_t  _crnt_im;   // current input method ID
  u_int16_t  _crnt_ic;   // current input context ID
  Atom       _server_atom;
  Atom       _xim_xconnect; 
  Atom       _xim_protocol; 
  Atom       _xim_moredata; 
