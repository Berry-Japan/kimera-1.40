...
AnthyEngine::init()
{
  ...
  if ( !_dlanthy ) {
    ...
    qDebug("Anthy version: %s", version.ascii());
    if (version >= "7100b") {
...
{
  DEBUG_TRACEFUNC("hira: %s", hira.local8Bit().data());
  if ( !_anthy )
    return FALSE;
...
AnthyEngine::getCandidate(int idx, QStringList& candidate)
{
  ...
  for (int i = 0; i < seg.nr_candidate; ++i) {
    ...
    if (len > 0) {
      ...
      if ( !str.isEmpty() && candidate.find(str) == candidate.end()) {
	candidate << str;
      }
    }
  }
...
AnthyEngine::timerEvent(QTimerEvent* e)
{
  ...
  qDebug("hiragana: %s", _srcpredict.local8Bit().data());
